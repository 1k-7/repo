...
from info import (PICS, TUTORIAL, ADMINS, URL, MAX_BTN, BIN_CHANNEL,
                  ...
                  SECOND_FILES_DATABASE_URL # Import for stats/checks
                  )
...
from utils import (get_size, is_subscribed, is_check_admin, get_wish,
                   ...
                   get_settings, save_group_settings, # Use async wrappers now
                   get_verify_status, update_verify_status) # Use async wrappers now
...
async def group_search(client, message: Message):
    ...
    if not user_id: return # Ignore anonymous
...
async def next_page(bot, query: CallbackQuery):
    ...
    if not search or not cap: await query.answer("ʀᴇǫᴜᴇsᴛ ᴇxᴘɪʀᴇᴅ.", show_alert=True); try: await query.message.edit("❌") except: pass; return
    ...
    settings = await get_settings(query.message.chat.id) # await
...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    current_offset = 0 # Start filtered list from 0
    # ** FIX: Pass query positionally **
...
async def quality_search(client: Client, query: CallbackQuery):
    ...
    current_offset = 0
    # ** FIX: Pass query positionally **
    files, next_filtered_offset, total_filtered = await get_search_results(query=original_search, lang=qual, offset=current_offset) # Use lang for quality
...
async def advantage_spoll_choker(bot, query: CallbackQuery):
    ...
    try:
        movie = await get_poster(id, id=True) # await
    ...
    # ** FIX: Pass query positionally **
    files, offset, total_results = await get_search_results(query=search_query, offset=0) # Use async
    if files:
        k = (search_query, files, offset, total_results) # Package
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if data == "close_data":
        ...
        ident, mc = data.split("#")
        btn = await is_subscribed(client, query) # await
        if btn:
        ...
        async def get_stat_safe(func, *args): # Helper
            try: call_func = partial(func, *args) if args else func; result = await loop.run_in_executor(None, call_func); return result
        ...
        try: await sts_msg.edit_media( media=InputMediaPhoto(random.choice(PICS), caption=stats_text), reply_markup=InlineKeyboardMarkup(buttons) )
             ...
             logger.error(f"Error editing final stats: {e_final_edit}")
             try: await sts_msg.edit(stats_text, reply_markup=InlineKeyboardMarkup(buttons)) # Fallback edit text
             except: await query.answer("Error displaying stats.", show_alert=True); return
    ...
    # --- Settings Callbacks (Use async utils) ---
        ...
        new_status = not (status == "True"); await save_group_settings(grp_id, set_type, new_status) # await save
        ...
        try:
            ...
            await save_group_settings(grp_id, value_key, v1) # await save
            if api_key and v2: await save_group_settings(grp_id, api_key, v2) # await save
        ...
        if isinstance(setting_info, list): [await save_group_settings(grp_id, k, dv) for k, dv in setting_info] # await saves
        else: k, dv = setting_info; await save_group_settings(grp_id, k, dv) # await save
        back_btn = [[ InlineKeyboardButton('« ʙᴀᴄᴋ', callback_data=f'{setting_type}_setgs#{grp_id}') ]]
    ...
    # --- Moderation Callbacks ---
        ...
        elapsed = get_readable_time(time_now() - start); final = f"✅ `{action} {target}` ᴄᴏᴍᴘʟᴇᴛᴇᴅ ɪɴ {elapsed}.\n\n♢ sᴜᴄᴄᴇss: <code>{success}</code>\n♢ ᴇʀʀᴏʀs: <code>{errors}</code>"
    ...
    # Fallback
    else: logger.warning(f"Unhandled CB data: {data}"); await query.answer("❓", show_alert=False)


async def auto_filter(client, msg, s, spoll=False):
    if not spoll:
        message = msg; settings = await get_settings(message.chat.id) # await
        search = re.sub(r"\s+", " ", re.sub(r"[-:\"';!]", " ", message.text)).strip()
        if not search: return await s.edit("ᴘʟᴇᴀsᴇ ᴘʀᴏᴠɪᴅᴇ ᴛᴇxᴛ.")
        # ** FIX: Pass query positionally **
        files, offset, total_results = await get_search_results(query=search, offset=0) # await
        if not files:
            if settings.get("spell_check", True): return await advantage_spell_chok(message, s) # await
            else: return await s.edit(script.NOT_FILE_TXT.format(message.from_user.mention, search))
    else: # From spell check
    ...
    try:
        if imdb and imdb.get('poster'):
             ...
             except (MediaEmpty, PhotoInvalidDimensions, WebpageMediaEmpty) as e_media: logger.warning(f"IMDb poster fail {search}: {e_media}"); k = await message.reply_text(final_caption, reply_markup=reply_markup, disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML, quote=True)
             except FloodWait as e_flood: logger.warning(f"Flood photo: {e_flood.value}"); await asyncio.sleep(e_flood.value); k = await message.reply_text(final_caption, reply_markup=reply_markup, disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML, quote=True)
             except Exception as e_photo: logger.error(f"Send photo error: {e_photo}"); k = await message.reply_text(final_caption, reply_markup=reply_markup, disable_web_page_preview=True, parse_mode=enums.ParseMode.HTML, quote=True)
    ...
    except FloodWait as e_main: logger.warning(f"Flood main reply: {e_main.value}"); await asyncio.sleep(e_main.value)
    except Exception as e_main: logger.error(f"Final filter error: {e_main}", exc_info=True); await s.edit("❌ ᴇʀʀᴏʀ.")
